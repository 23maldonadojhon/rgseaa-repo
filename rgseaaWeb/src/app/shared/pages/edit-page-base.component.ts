...
import {AppError} from '@libs/commons';
import {FormMapper} from '@libs/commons/form';
...
@Directive()
export abstract class EditPageBaseComponent<T, F = T> implements OnInit, OnDestroy {
   ...
   */
  protected _srcData: T | undefined;
  /**
   * Formatted form source data.
   */
  private _startValue: F | undefined;

  protected _action : string ='none';

  protected _idIndustry : number = 0;
  ...
  constructor(
    ...
    protected crudServiceUtil :CrudImplService
  ) {
  ...
  ngOnInit() {
    ...
    this.loadResourceId().subscribe(() => {

        this._action = this.route.snapshot.params["action"];
        this._idIndustry = this.route.snapshot.params["idIndustry"];
  ...
  protected afterLoadDataSuccess(startValue: F) {
      ...
      if(this._action==='copy'){
          ...
          this.notification.show({ message: 'text.other.dataCopy' });
      }
  ...
  protected createSaveOperation(payload: T) {
    ...
    if (this.isNew || this._action==='copy') {
