...
import {VAppSampleTemplate} from '@libs/sdk/sample-template';
import {AppScopeView} from '@libs/sdk/scope';
...
export class AppContextService {
  ...
  private readonly activeScope = new BehaviorSubject<AppScopeView | undefined>(undefined);
  private readonly activeSeason = new BehaviorSubject<SampleSeason | undefined>(undefined);
  private readonly activeTemplate = new BehaviorSubject<VAppSampleTemplate | undefined>(undefined);
  ...
  private readonly availableScopes = new BehaviorSubject<AppScopeView[]>([]);
  private readonly availableSeasons = new BehaviorSubject<SampleSeason[]>([]);
  private readonly availableTemplates = new BehaviorSubject<VAppSampleTemplate[]>([]);
  ...
  useScope(value: AppScopeView | undefined) {
  ...
  useTemplate(value: VAppSampleTemplate | undefined) {
  ...
  private scopesLoader() {
    ...
    return this.crudService.findAll<AppScopeView>(config).pipe(
      map(page => page.content)
    );
  }
  ...
  private templateLoader() {
    ...
    return this.crudService.findAll<VAppSampleTemplate>(config).pipe(
      map(page => page.content),
      map(list => {
        const userId = this.authContext.instant().getDetails().userId;
